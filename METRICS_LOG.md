...
## Этап 5 (+27 новых лиг из xgabora) — 2026-07-17

**Дата**: 2026-07-17
**Изменения**: +27 новых лиг (RUS, BRA, CHN, JAP, MEX, USA, SC0-3, E2-3, D2, F2, I2, SP2, EC, ARG, AUT, DEN, FIN, IRL, NOR, POL, ROM, SUI, SWE) — 141,183 матчей из xgabora dataset.
**Источник**: база для обучения/Matches.csv (xgabora, 230K matches, 25 years)
**Дедупликация**: 89,370 точных дублей отброшены (10 существующих лиг, 100% пересечение)

### Метрики (held-out test set 20%)

| Метрика | Этап 4 (70 фич) | Этап 5 (70 фич, +27 лиг) | Дельта |
|---------|-----------------|--------------------------|--------|
| Dataset | 30,989 rows, 16 лиг | 48,290 rows, 41 лига | +56% rows, +25 лиг |
| Test Accuracy | 51.86% | **51.19%** | -0.67% |
| Test Log-loss | 1.058 | **1.022** | **-0.036 (лучше)** |
| Away Win P/R | 0.48/0.56 | 0.47/0.54 | -0.02/-0.02 |
| Draw P/R | 0.31/0.08 | 0.32/0.02 | +0.01/-0.06 |
| Home Win P/R | 0.56/0.74 | 0.54/0.76 | -0.02/+0.02 |
| ROI (edge ≥2%) | -6.93% | -7.93% | -1.00% |
| ROI (edge ≥5%) | -7.31% | -6.72% | +0.59% |
| ROI (edge ≥10%) | -9.60% | -9.73% | -0.13% |
| Odds-subset | 1,151 bets | 2,776 bets | +141% bets |

### Per-league (top performers)

| League | N | Accuracy | Note |
|--------|---|----------|------|
| WC_2026 | 83 | 69.88% | International tournament |
| Kazakhstan_1D | 60 | 68.33% | Weak league, high variance |
| International | 1,984 | 60.89% | Mix of national teams |
| Belarus_1D | 137 | 56.93% | |
| MLS | 165 | 55.76% | |

### Анализ

**Позитив**:
- Log-loss улучшился (-0.036) — модель лучше калибрована на расширенных данных
- Odds-subset вырос на 141% (1,151→2,776 ставок) — больше данных для ROI анализа
- 25 новых лиг доступны для предсказаний (расширение покрытия)
- Dataset вырос на 56% (31K→48K матчей)

**Негатив**:
- Accuracy чуть просела (-0.67%) — новые лиги имеют marginal signal (48.7% в валидации)
- Draw recall деградировал (0.08→0.02) — модель почти не предсказывает ничьи на новых лигах
- ROI (edge ≥2%) ухудшился (-1.00%) — больше "шумных" ставок на слабых лигах
- Все новые лиги — global fallback (ни одна не прошла MIN_LEAGUE_TRAIN_ROWS)

**Вывод**: Добавление 27 новых лиг улучшило калибровку (log-loss) и расширило покрытие, но незначительно ухудшило accuracy и ROI. Основная ценность — расширение доступных лиг для предсказаний, а не улучшение точности. Новые лиги не добавляют значимого сигнала для модели.

*Baseline ПЕРЕД добавлением новых лиг (Этап 4, 70 фич, 16 лиг) — accuracy 51.86%, log-loss 1.058.*

---

## Пилотный тест O/U + BTTS implied (2026-07-18)

### Баг: неправильная линия тотала в пилотном тесте

**Проблема:** Пилотный тест читал данные O/U напрямую из `sstats_odds` таблицы, минуя `consensus_over_under()`. Парсер брал **последнюю** запись Over/Under для каждого матча — это оказывалась line 5.5 (а не 2.5).

**До (line 5.5):** implied_over = 0.187 → точность 47.9% (хуже baseline)
**После (line 2.5):** implied_over = 0.693 → точность **55.9%** (+3.8% над baseline)

**Корневая причина:** В `sstats_odds` таблице 290 строк O/U для одного матча (14 букмекеров × ~20 линий). Парсер брал последнюю запись без фильтрации по line.

### Production-код НЕ затронут

Все использования O/U в production проходят через `consensus_over_under()` (scrapers/sstats.py:411), который **корректно** группирует по line и возвращает все линии. AI-анализ показывает пользователю ВСЕ доступные линии. Баг существовал **только в пилотном тесте**.

### Исправленные результаты (4720 матчей, line 2.5)

| Метрика | Значение | Baseline |
|---------|----------|----------|
| O/U accuracy (>2.5 goals) | **55.91%** | 52.12% |
| BTTS accuracy | **55.93%** | 54.09% |
| O/U → home_win correlation | +0.104 | — |
| O/U → draw correlation | -0.083 | — |
| LR accuracy (O/U+BTTS → 1X2) | 43.90% | 43.90% |

**Вывод:** O/U line 2.5 помогает предсказывать **тотал** (+3.8%), но не **исход** (1X2). Для текущей модели (1X2) — низкий приоритет. Для будущей модели тотала — перспективно.

*Примечание: тест проведён как correlation/LR на 4720 матчах. Multi-seed XGBoost/LightGBM тест не проведён — рекомендуется при расширении покрытия sstats данных.*
