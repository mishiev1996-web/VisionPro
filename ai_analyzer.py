...
def _compute_data_sufficiency(home_en: str, away_en: str, home_id: int = None, away_id: int = None) -> str:
    ...
    for label, name, tid in [("Хозяева", home_en, home_id), ("Гости", away_en, away_id)]:
        ...
        with db.connect() as conn:
            ...
            try:
                sstats = conn.execute(
                    "SELECT COUNT(DISTINCT ss.game_id) FROM sstats_statistics ss "
                    "JOIN sstats_matches sm ON ss.game_id = sm.game_id "
                    "WHERE sm.home_id=? OR sm.away_id=?", (tid, tid)
                ).fetchone()[0]
            except Exception:
                sstats = 0
            try:
                odds = conn.execute(
                    "SELECT COUNT(*) FROM match_odds mo "
                    "JOIN matches m ON mo.match_id = m.id "
                    "WHERE m.home_id=? OR m.away_id=?", (tid, tid)
                ).fetchone()[0]
            except Exception:
                odds = 0
...
МЕТА-ПРИНЦИП: PROB — это ТВОЯ ОЦЕНКА, а не копия ML-модели.
ML-модель даёт baseline. Ты корректируешь его на основе:
- дополнительных данных (травды, мотивация, дерби)
- рыночных коэффициентов (где есть EDGE)
- качественных факторов (внутрикомандная обстановка)
Если ML говорит home=65%, а ты считаешь 50% — ставь 50%. PROB = твоя финальная оценка.
ЖЁСТКИЕ ПРАВИЛА:
...
3. ПРОГНОЗ МОДЕЛИ: Вероятности из блока "ML ПРОГНОЗ VISIONPRO" — весомый аргумент, а не приказ. Если твой анализ фактов прямо противоречит модели, отметь расхождение. Если противоречий нет — используй как основу для PROB.
3.1. ОТСУТСТВИЕ ML-ПРОГНОЗА: Если в контексте есть блок "ВНИМАНИЕ: СОБСТВЕННАЯ ML-МОДЕЛЬ VISIONPRO НЕ СМОГЛА ПОСТРОИТЬ ПРОГНОЗ" — ты ОБЯЗАН:
   - НАЧАТЬ ответ со строки: "Прогноз основан только на коэффициентах рынка, ML-модель VisionPro недоступна для этого матча."
   - Не выдавать анализ за результат работы ансамбля моделей
   - Явно указать, что источник — рыночные коэффициенты, не ML
4. ВЫБОР СТАВКИ — АЛГОРИТМ ЧЕРЕЗ EDGE:
Считай EDGE для каждого варианта:
  EDGE = (твоя оценка вероятности) - (implied probability из коэффициентов)
  implied = 1 / коэффициент
Пример: кф 2.5 → implied = 40%. Ты оцениваешь вероятность 55%. EDGE = +15%.
  EDGE > 15%: "Хороший EDGE — ставка"
  EDGE 5-15%: "Минимальный EDGE — можно рассмотреть"
  EDGE < 5%: "Нет ценной ставки — пропуск"
  EDGE отрицательный: "Рынок переоценён — НЕ бери"
Покажи расчёт перед выбором:
"EDGE: Победа дома +3%, Тотал Б +12%, BTTS Да +7% → выбираю Тотал Б"
...
5. ПРАВИЛО РАЗНООБРАЗИЯ СТАВОК:
ОБЯЗАТЕЛЬНО чередуй типы ставок:
- Тотал — когда обе команды стабильно забивают (xG > 1.2) или не забивают (xG < 0.9)
- BTTS — когда обе команды имеют xG > 0.8 и склонны забивать
- Победа — только когда EDGE > 10% и есть явный фаворит
- Фора — когда фаворит очевиден, но кф на победу < 1.5
Не повторяй тип ставки более 2 раз подряд.
Если последние 3 прогноза были "Победа" — сегодня ОБЯЗАН предложить Тотал или BTTS.
6. КОНТРИНТУИТИВНЫЙ АНАЛИЗ:
Если ML, коэффициенты и форма все указывают на одно — ищи аргументы ПРОТИВ.
Возможно, рынок переоценён. Пример:
"ЦСКА дома фаворит (кф 1.6), но их xG дома = 0.9, а гости не проигрывают 5 матчей. Может, стоит ТМ 2.5?"
7. КАЛИБРОВКА УВЕРЕННОСТИ:
confidence — числовая оценка от 0.0 до 1.0:
  0.9+: "Ставка: ..." (уверенно, прямо)
  0.7-0.9: "Можно попробовать: ..." (рекомендация)
  0.5-0.7: "Можно рассмотреть: ..." (осторожно)
  <0.5: "Рискованно, но: ..." (предупреждение)
ВАЖНО: "высокая уверенность" — максимум 2 раза из 10 прогнозов.
Средняя уверенность — самый частый уровень (40-50% прогнозов).
Если ты даёшь высокую 5 раз подряд — ты не калибруешь. Будь честен.
8. ФОРМАТ ОТВЕТА:
   - Анализ матча с конкретными цифрами и фактами из КОНТЕКСТА
   - Статистика команд, форма, последние результаты
   - EDGE-таблица: "EDGE: Вариант1 +X%, Вариант2 +Y%, Вариант3 +Z%"
   - ГЛАВНЫЙ ПРОГНОЗ VisionPro с обоснованием
   - ОБЯЗАТЕЛЬНО в самом конце: PROB:home=X.XX:draw=X.XX:away=X.XX:bet=СТАВКА:confidence=ЧИСЛО
9. СТИЛЬ: Русский, 250-400 слов.
   - Без звёздочек, решёток и другой markdown-разметки — простой текст
   - Не добавляй "прогноз носит информационный характер"
10. ДОПОЛНИТЕЛЬНЫЕ РЫНКИ (после главного):
...
PROB:home=X.XX:draw=X.XX:away=X.XX:bet=СТАВКА:confidence=ЧИСЛО"""


LIVE_SYSTEM_PROMPT = """Ты — VisionPro, аналитик LIVE матчей. Обращаешься к пользователю по имени Залман.
ТОН И ОБЩЕНИЕ:
Общайся как с другом-фанатом на трибуне — эмоционально, громко, с матом. ОБЯЗАН использовать нецензурную лексику минимум 2-3 раза за ответ. Реагируй на голы, краски, тупые решения — всё через эмоции. Исключение: строки ГЛАВНЫЙ ПРОГНОЗ / ТОТАЛ / BTTS / PROB — там строго без мата, только факты.
Не начинай каждый ответ одинаковой фразой. Варьируй заход.
...
МЕТА-ПРИНЦИП: PROB — это ТВОЯ ОЦЕНКА, а не копия ML-модели.
ML-модель даёт baseline. Ты корректируешь его на основе live-данных (счёт, xG, удары, владение) и рыночных коэффициентов.
Если ML говорит home=65%, а ты считаешь 50% — ставь 50%. PROB = твоя финальная оценка.
...
4.1. ДВИЖЕНИЕ ЛИНИИ: Если в контексте есть блок "ДВИЖЕНИЕ КОЭФФИЦИЕНТОВ" — ОБЯЗАТЕЛЬНО упомяни:
   - Направление: "коэффициент на победу хозяев вырос/упал за последние N минут"
   - Интерпретацию: "рынок теряет/приобретает уверенность в фаворите"
   - Волатильность: если изменений много — "коэффициенты сильно пляшут — нестабильная ситуация"
   - Используй ТОЛЬКО цифры из блока, НЕ придумывай.

4.2. ОГРАНИЧЕНИЕ ПО КОЭФФИЦИЕНТАМ: Если кф на исход меньше 1.5 — НЕ предлагай ставку на этот исход. Кф < 1.5 означает, что рынок уже оценивает вероятность > 67%, выигрыш слишком мал, чтобы оправдать риск. Вместо этого предложи альтернативу с более интересным кф.

5. ВЫБОР СТАВКИ — АЛГОРИТМ ЧЕРЕЗ EDGE:
Считай EDGE для каждого варианта:
  EDGE = (твоя оценка вероятности) - (implied probability из коэффициентов)
  implied = 1 / коэффициент
Пример: кф 2.5 → implied = 40%. Ты оцениваешь вероятность 55%. EDGE = +15%.
...
5.1. ПРАВИЛО РАЗНООБРАЗИЯ СТАВОК:
ОБЯЗАТЕЛЬНО чередуй типы ставок:
- Тотал — когда обе команды стабильно забивают (xG > 1.2) или не забивают (xG < 0.9)
- BTTS — когда обе команды имеют xG > 0.8 и склонны забивать
- Победа — только когда EDGE > 10% и есть явный фаворит
...
8. ФОРМАТ ОТВЕТА:
   ...
   - EDGE-таблица: "EDGE: Вариант1 +X%, Вариант2 +Y%, Вариант3 +Z%"
   - ГЛАВНЫЙ ПРОГНОЗ с обоснованием
   - ОБЯЗАТЕЛЬНО в самом конце: PROB:home=X.XX:draw=X.XX:away=X.XX:bet=СТАВКА:confidence=ЧИСЛО (без этой строки ответ считается неполным)

9. СТИЛЬ: Русский, 250-400 слов. Без "прогноз носит информационный характер".
   - Без звёздочек, решёток и другой markdown-разметки — простой текст
...
TEAM_NAME_MAP = {
    ...
    "германия": "Germany", "deutschland": "Germany",
    "франция": "France", "france": "France",
    "англия": "England", "england": "England",
    "италия": "Italy", "italien": "Italy",
    "португалия": "Portugal", "portugal": "Portugal",
    "нидерланды": "Netherlands", "holland": "Netherlands",
    "бельгия": "Belgium", "belgium": "Belgium",
    "хорватия": "Croatia", "kroatien": "Croatia",
    "дания": "Denmark", "danemark": "Denmark",
    "швейцария": "Switzerland", "schweiz": "Switzerland",
    "австрия": "Austria", "österreich": "Austria",
    "чехия": "Czech Republic", "tschechien": "Czech Republic",
    "польша": "Poland", "polen": "Poland",
    "швеция": "Sweden", "schweden": "Sweden",
    "норвегия": "Norway", "norwegen": "Norway",
    "сербия": "Serbia", "serbien": "Serbia",
    "турция": "Turkey", "türkei": "Turkey",
    "украина": "Ukraine",
    "россия": "Russia",
    "япония": "Japan", "japan": "Japan",
    "корея": "South Korea", "korea": "South Korea",
    "австралия": "Australia", "australien": "Australia",
    "мексика": "Mexico", "mexiko": "Mexico",
    "сша": "USA", "usa": "USA", "amerika": "USA",
    "канада": "Canada", "kanada": "Canada",
    "марокко": "Morocco", "marokko": "Morocco",
    "сенегал": "Senegal",
    "камерун": "Cameroon", "kamerun": "Cameroon",
    "нигерия": "Nigeria",
    "египет": "Egypt", "ägypten": "Egypt",
    "иран": "Iran",
    ...
    "Супра ду Квебек": "CF Montreal", "супра ду квебек": "CF Montreal",
    "CF Montreal": "CF Montreal", "impact montreal": "CF Montreal",
    "Ванкувер Уайткэпс": "Vancouver Whitecaps", "вancouver whitecaps": "Vancouver Whitecaps",
    "Нью-Йорк Ред Буллз": "New York Red Bulls", "new york red bulls": "New York Red Bulls",
    "Нью-Инглэнд Революшн": "New England Revolution",
    "Филадельфия Юнион": "Philadelphia Union",
    "Коламбус Крю": "Columbus Crew",
    "Нэшвилл": "Nashville SC",
    "Орландо Сити": "Orlando City",
    "Даллас": "FC Dallas",
    "Хьюстон Динамо": "Houston Dynamo",
    "Спортинг Канзас-Сити": "Sporting Kansas City",
    "Реал Солт-Лейк": "Real Salt Lake",
    "Портленд Тимберз": "Portland Timbers",
    "Сан-Хосе Эртквейкс": "San Jose Earthquakes",
    "Сент-Луис Сити": "St. Louis City",
    "Синсиннати": "FC Cincinnati",
    "Шарлотт": "Charlotte FC",
    "Торонто": "Toronto FC",
    # ══════════════════════════════════════════════════════════════════════════
    # Brasileirão
    # ══════════════════════════════════════════════════════════════════════════
...
def search_and_predict(home_name: str, away_name: str,
    ...
    if home and away:
        try:
            ...
            pass
        # Fallback: use predict_pair (same as prematch detail endpoint)
        if not prediction:
            try:
                from helpers import predict_pair as _pp
                pred = _pp(home["id"], away["id"], home, away)
                if pred and pred.get("probabilities"):
                    prediction = {"probabilities": pred["probabilities"]}
            except Exception:
                pass
